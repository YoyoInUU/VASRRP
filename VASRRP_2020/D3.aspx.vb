...
Partial Class D6
    ...
    Sub loadoption(ByVal stem As System.Data.DataTable, ByVal nowpage As Integer)
        ...
        Ltext.Text = stem.Rows((nowpage - 1) * 3 + 0).Item("Questions")
        Lanchor.Text = stem.Rows((nowpage - 1) * 3 + 0).Item("Left anchor")
        Ranchor.Text = stem.Rows((nowpage - 1) * 3 + 0).Item("Right anchor")

        stem.DefaultView.RowFilter = "osn >" & (nowpage - 1) * 3 + 0 & " and osn <=" & (nowpage - 1) * 3 + 3 & ""
    ...
    Protected Sub Bun_submit_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles Bun_submit.Click
        ...
        Dim checkrow() As Data.DataRow = stemt.Select("osn='" & nowpages * 3 & "'")
